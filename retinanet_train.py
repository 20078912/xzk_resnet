...
class WrapTransformDataset(torch.utils.data.Dataset):
    """Apply a transform to image only, keep target."""
...
class SyntheticDetectionDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for _ in range(num_obj):
            ...
            x2 = x1 + w; y2 = y1 + h
            c = random.randint(0, self.num_classes - 1)  # 0..K-1
...
class CocoWrapper(data.Dataset):
    """
    Wrap torchvision.datasets.CocoDetection to return {'boxes','labels'}.
    Expects bbox in [x,y,w,h], category_id in [1..num_classes] or arbitrary;
    we remap to contiguous 0..K-1.
    """
    def __init__(self, img_root, ann_file, transforms=None):
        ...
        self.cat_id_to_contig = {cid: i for i, cid in enumerate(cat_ids)}  # 0-based labels

    def __len__(self): return len(self.ds)

    def __getitem__(self, idx):
        img, anns = self.ds[idx]  # img: PIL.Image, anns: list
        w, h = img.size
        boxes = []; labels = []
        for a in anns:
            if a.get("iscrowd", 0) == 1:  # skip crowd
                continue
            x, y, bw, bh = a["bbox"]
            if bw <= 1 or bh <= 1:
                continue
...
# =========================
# YOLO .txt wrapper (images/ + labels/)
# =========================
class YoloTxtWrapper(data.Dataset):
    """
    Expect:
      img_root/   *.jpg|*.png
      labels_dir/ same-stem .txt with lines: <cls> <cx> <cy> <w> <h> (normalized 0..1)
    """
    def __init__(self, img_root, labels_dir, transforms=None):
        ...
        if txt_path.exists():
            with open(txt_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    labels.append(int(c))  # YOLO是0-based，正好符合RetinaNet
...
def compute_map50(preds: List[Dict], gts: List[Dict], iou_thr=0.5, num_classes=1):
    ...
    for cls in range(0, num_classes):
        ...
        for p, gt in zip(preds, gts):
            pi = (p["labels"] == cls)
            gi = (gt["labels"] == cls)
            if pi.sum() == 0 and gi.sum() == 0:
                continue
            cls_preds.append({"boxes": p["boxes"][pi], "scores": p["scores"][pi]})
            cls_gts.append({"boxes": gt["boxes"][gi]})
        flat_preds = []
        for img_id, cp in enumerate(cls_preds):
            for b, s in zip(cp["boxes"], cp["scores"]):
                flat_preds.append((img_id, s.item(), b))
        flat_preds.sort(key=lambda x: -x[1])
        tp = []; fp = []; total_gts = sum([len(x["boxes"]) for x in cls_gts])
        matched = [set() for _ in cls_gts]
        for img_id, score, box in flat_preds:
            gt_boxes = cls_gts[img_id]["boxes"] if img_id < len(cls_gts) else torch.empty((0, 4))
            if len(gt_boxes) == 0:
                fp.append(1); tp.append(0); continue
            ious = box_iou(box.unsqueeze(0), gt_boxes).squeeze(0)
            best_i = torch.argmax(ious).item()
            best_iou = ious[best_i].item()
            if best_iou >= iou_thr and best_i not in matched[img_id]:
                matched[img_id].add(best_i)
                tp.append(1); fp.append(0)
            else:
                fp.append(1); tp.append(0)
        if total_gts == 0:
            # 没有GT的类，按0计入（如需跳过，可改成 continue）
            ap_per_class[cls] = 0.0
            continue
        tp = np.array(tp); fp = np.array(fp)
        tp_cum = np.cumsum(tp); fp_cum = np.cumsum(fp)
        recalls = tp_cum / (total_gts + eps)
        precisions = tp_cum / (tp_cum + fp_cum + eps)
        ap = 0.0
        for r_th in np.linspace(0, 1, 11):
            p = precisions[recalls >= r_th].max() if np.any(recalls >= r_th) else 0.0
            ap += p
        ap /= 11.0
        ap_per_class[cls] = ap
...
def build_retinanet(num_classes=12, min_anchor=16, aspect_ratios=(0.5, 1.0, 2.0),
    ...
    # 兼容不同torchvision版本：优先v2，退回v1
    model = None
    try:
        model = tv.models.detection.retinanet_resnet50_fpn_v2(weights=None, num_classes=num_classes)
    except Exception:
        model = tv.models.detection.retinanet_resnet50_fpn(weights=None, num_classes=num_classes)

    # anchor sizes per FPN level (start small for tiny objects)
    sizes = []
    s = min_anchor
    for _ in range(5):
        sizes.append((s,))
...
def train_one_epoch(model, loader, optimizer, device, epoch, grad_accum=1, lr_scheduler=None, log_interval=20):
    ...
    return loss_meter / max(1, len(loader)), elapsed
...
def main():
    ...
    # dataset mode
    parser.add_argument('--synthetic', action='store_true', help='Use synthetic toy data (smoke test)')
    parser.add_argument('--yolo-txt', action='store_true', help='Use YOLO .txt dataset loader')
    # yolo paths
    parser.add_argument('--data-root', type=str, default='', help='Images root directory')
    parser.add_argument('--labels-dir', type=str, default='', help='YOLO labels directory (txt files)')
    # coco paths
    ...
    parser.add_argument('--workers', type=int, default=0)  # Windows更稳
    ...
    if args.synthetic:
        ...
        train_tf = tv.transforms.Compose([train_aug])
        val_tf   = tv.transforms.Compose([])
        train_ds = WrapTransformDataset(train_ds, train_tf)
        val_ds   = WrapTransformDataset(val_ds, val_tf)
    else:
        resize = ResizeShortSide(args.short_side, args.max_size)
        if args.yolo_txt:
            assert args.data_root and args.labels_dir, \
                "For --yolo-txt, set --data-root (images dir) and --labels-dir (txt dir)."
            # 先用同一目录做train/val冒烟；严格实验请自己切分
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        print(f"[epoch {epoch}] loss={loss_avg:.4f}  mAP50={metrics['mAP50']:.4f}  "
