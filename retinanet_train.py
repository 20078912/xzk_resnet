# retinanet_train.py (with logging + visualization integrated)
...
train_aug = tv.transforms.Compose([ tv.transforms.RandomHorizontalFlip(p=0.5), ])
...
def build_retinanet(num_classes=12, min_anchor=16, aspect_ratios=(0.5, 1.0, 2.0),
    ...
    sizes = []; s = min_anchor
    for _ in range(5): sizes.append((s,)); s *= 2
...
def build_scheduler(optimizer, epochs, cosine=False, warmup_epochs=0):
    ...
    def warmup_lambda(cur_epoch):
        if cur_epoch >= warmup_epochs: return 1.0
...
def main():
    ...
    with open(log_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["epoch", "train_loss", "mAP50", "precision", "recall", "train_time", "eval_time"])
    print(f"[Init] Logging to {log_path}")

    # =========================
    # Datasets & loaders (same as before)
    # =========================
    if args.synthetic:
        ...
        if args.yolo_txt:
            if args.eval_only:
                ...
                val_ds = YoloTxtWrapper(args.val_images, args.val_labels,
                    transforms=tv.transforms.Compose([resize]), num_classes=args.num_classes)
            else:
    ...
    if args.resume and os.path.isfile(args.resume):
        ...
        cosine_ckpt = bool(ckpt.get('args', {}).get('cosine', False))
        warmup_ckpt = int(ckpt.get('args', {}).get('warmup_epochs', 0))
        epochs_ckpt = int(ckpt.get('args', {}).get('epochs', args.epochs))
        try:
            lr_sched = build_scheduler(optimizer, epochs=epochs_ckpt,
                                       cosine=cosine_ckpt, warmup_epochs=warmup_ckpt)
            if 'lr_sched' in ckpt: lr_sched.load_state_dict(ckpt['lr_sched'])
        except Exception as e:
            print(f"[warn] scheduler mismatch: {e}")
            lr_sched = build_scheduler(optimizer, epochs=args.epochs,
                                       cosine=args.cosine, warmup_epochs=args.warmup_epochs)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # ==== 日志记录 ====
        ap50_per_class = metrics.get("AP50_per_class", {})
        precision = np.mean(list(ap50_per_class.values())) if ap50_per_class else 0
        recall = precision  # 暂用相同值，若有单独 recall 可替换
        with open(log_path, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([epoch, f"{loss_avg:.4f}", f"{metrics['mAP50']:.4f}",
                             f"{precision:.4f}", f"{recall:.4f}", f"{train_time:.1f}", f"{eval_time:.1f}"])

        # ==== 保存检查点 ====
        ckpt = {
            "epoch": epoch, "model": model.state_dict(),
            "optimizer": optimizer.state_dict(), "lr_sched": lr_sched.state_dict(),
            "args": vars(args), "mAP50": metrics["mAP50"]
        }
        ...
        if lr_sched is not None: lr_sched.step()
    ...
    try:
        ...
        plt.title("Validation mAP Curve"); plt.legend(); plt.grid()
        ...
        plt.title("Precision & Recall Curve"); plt.legend(); plt.grid()
        plt.savefig(os.path.join(args.out_dir, "pr_curve.png"), dpi=300)

        print("✅ Charts saved: loss_curve.png, map_curve.png, pr_curve.png")
